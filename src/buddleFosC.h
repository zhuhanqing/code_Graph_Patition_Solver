...
namespace buddleFosCSolver
{
    ...
    {
        //int seedRank{0}; //default = 0 and store seed rank 
        //TIndex seedInfo{0}; //default = 0 and store seed sort 
        //Vector<TIndex, TValue> loadDrainVec;
    ...
    void loadDiffusion(GraphParser& gp, const CsrMatrix<TIndex, TValue>& graph, Vector<TIndex, TData>& partResPie,TIndex numOfPartition, TIndex numOfIteration, TIndex numOfConsolidation, TValue cgTolerance)
    {
        ...
        //TValue cgTolerance = 1e-11;
        TValue loadPerSeed = 6; //decide by yourself; no effect on algorithm itself
        ...
        for(TIndex iteration = 0;iteration < numOfIteration; ++iteration)
        {
            ...
            // if not define pie need random select seed 
            ...
            {   
                std::cout<<consolidationCnt<<" consolidation"<<std::endl;
                ...
                TValue ratioMinMean = TValue(minPartNum)/numOfIndex * numOfPartition;
    ...
    void printPartitionResult(Vector<TIndex, TValue> &partitionResult,std::string& fileName)
    {
		...
		//more graph
		pathName= "/home/local/eda03/zixuan/NewPar/benchmark/" + fileName;
        ...
        TIndex printSize = partitionResult.size();

        for (TIndex i = 0;i < printSize; ++i)
        {
	...
	void checkCutSize(GraphParser& gp, Vector<TIndex, TValue>&partitionResult)
	{
        ...
        TIndex cutsize = 0;
		TIndex indexNum = partitionResult.size();
		TIndex indexPartitionNum,indexConnectIndexPartitionNum;
		TIndex connectSort, connectRange, connectRangeBegin;
