...
void loadDiffusion(GraphParser& gp, const CsrMatrix<TIndex, TValue>& graph, std::vector<TData>& partResPie,TIndex numOfPartition, TIndex numOfIteration, TIndex numOfConsolidation, TValue cgTolerance, TFlag seedGenFlag, TValue maxImbalance, int threadsNum)
{
    //set sparse matrix
    ...
    for(TIndex iteration = 0;iteration < numOfIteration; ++iteration)
    {
        ...
        {   
            ...
            for (TIndex partParallelCnt = 0; partParallelCnt < numOfPartition; ++partParallelCnt)
            {
                ...
                {
                    //break;
                    std::cout<<"error with a partitionNum = 0"<<std::endl;
                }
